...
<div id="grados-de-libertad-y-estadístico-f" class="section level1">
...
```
    ...
    ## x_peso     1 5265.8  5265.8  25.782 0.0002119 ***
    ## Residuals 13 2655.1   204.2                      
    ## ---
...
<div id="continuación-del-ejemplo.-criaturas-extrañas" class="section level1">
...
<div id="analogía-con-regresión-lineal-simple" class="section level2">
...
<div id="ajuste-del-modelo-con-variable-categórica" class="section level2">
...
```
    ...
    ## (Intercept)  66.22222    6.50513 10.180000 1.462260e-07
    ## SexoMA      -28.72222   10.28551 -2.792493 1.524722e-02
...
```
    ...
    ## # A tibble: 15 × 5
    ##    y_tall Sexo  MA_ID HE_ID y_hat3
    ##     <dbl> <chr> <dbl> <dbl>  <dbl>
    ##  1     12 MA        1     0   37.5
    ##  2     21 MA        1     0   37.5
    ##  3     43 MA        1     0   37.5
...
<div id="resumen-de-la-variabilidad-observada-en-los-datos" class="section level2">
...
```
    ...
    ## Response: y_tall
    ##           Df Sum Sq Mean Sq F value  Pr(>F)  
    ## Sexo       1 2969.9 2969.88   7.798 0.01525 *
    ## Residuals 13 4951.1  380.85                  
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
...
<div id="ejemplo-temperatura" class="section level1">
...
``` r
df_ma = dt2 %>% filter(PronosticoFrecuente=='Nublado')
df_he = dt2 %>% filter(PronosticoFrecuente=='Parcialmente Nublado')
msimple_ma = mean(df_ma$VelocidadViento)
msimple_he = mean(df_he$VelocidadViento)

gg4 = dt2 %>% ggplot(aes(x=PronosticoFrecuente,y=VelocidadViento)) + 
              geom_point(alpha=0.4) + 
              theme_minimal() + labs(y='Velocidad del viento')

msimple_t  = tibble(Pronostico=c('Nublado','Parcialmente Nublado'),
                    Respuesta=c(msimple_ma,msimple_he)) 

gg4 + geom_point(data=msimple_t,aes(x=Pronostico,y=Respuesta),
                 col='red',size=2,shape=10)
```
...
``` r
modelo_vel = lm(VelocidadViento ~ PronosticoFrecuente,dt2)
summary(modelo_vel)
```

    ## 
    ## Call:
    ## lm(formula = VelocidadViento ~ PronosticoFrecuente, data = dt2)
    ## 
    ## Residuals:
    ##      Min       1Q   Median       3Q      Max 
    ## -0.73684 -0.13684  0.06316  0.16393  0.56471 
    ## 
    ## Coefficients:
    ##                                         Estimate Std. Error t value Pr(>|t|)
    ## (Intercept)                              1.23529    0.07320   16.88  < 2e-16
    ## PronosticoFrecuenteParcialmente Nublado  0.40155    0.08806    4.56 3.06e-05
    ##                                            
    ## (Intercept)                             ***
    ## PronosticoFrecuenteParcialmente Nublado ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Residual standard error: 0.3018 on 53 degrees of freedom
    ## Multiple R-squared:  0.2818, Adjusted R-squared:  0.2682 
    ## F-statistic: 20.79 on 1 and 53 DF,  p-value: 3.058e-05
...
```
    ...
    ## Analysis of Variance Table
    ## 
    ## Response: VelocidadViento
    ##                     Df Sum Sq Mean Sq F value    Pr(>F)    
    ## PronosticoFrecuente  1 1.8938 1.89385  20.793 3.058e-05 ***
    ## Residuals           53 4.8272 0.09108                      
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
...
<div id="ejemplo-complementario" class="section level1">
