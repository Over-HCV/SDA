...
#
# Cuatro perillas y una semilla. Ninguna cambia el problema; todas cambian la
# respuesta que se obtiene, y esa distinción es el contenido de esta pantalla.
#
# `reinicios` solo hace algo en los métodos que pueden terminar en un óptimo
# local. El ACP lo ignora (converge siempre al mismo subespacio) y k-medias
# no: ahí es la diferencia entre un resultado y una casualidad.
#
...
controles_control <- function(ns) {
  shiny::tagList(
    ...
    shiny::sliderInput(ns("reinicios"), "Reinicios", min = 1, max = 25,
                       value = 1, step = 1),
  ...
  bslib::card(
    ...
    bslib::card_body(
      ...
      shiny::tags$dl(
        ...
        shiny::tags$dd(class = "col-sm-9",
          paste("Fija el arranque. Cambia el camino; en un problema bien",
                "condicionado no cambia el destino. Que eso sea así es",
                "comprobable: cambiala y mirá la traza.")),
        shiny::tags$dt(class = "col-sm-3", "Reinicios"),
        shiny::tags$dd(class = "col-sm-9",
          paste("Cuántas veces se reintenta desde otro arranque, quedándose",
                "con el mejor. Con 1 estás confiando en la suerte del primer",
                "arranque; el método que no puede caer en un óptimo local lo",
                "ignora y no cuesta nada.")))))
}
